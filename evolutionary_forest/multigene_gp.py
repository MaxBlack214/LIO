...
class MultipleGeneGP():
    ...
    def __init__(self, content, gene_num, tpot_model: TPOTBase = None, partition_scheme=None,
        ...
        # print(f"content.keywords: {content.keywords}")
        ...
        for i in range(self.gene_num):
            ...
            if isinstance(pset, MultiplePrimitiveSet):
                if hasattr(pset, 'layer_mgp') and hasattr(pset, 'mgp_scope'):
                    ...
                    self.mgp_mode = True
                #剪枝修改代码开始
                #self.gene.append(PrimitiveTree(content(pset=pset.pset_list[i])))#剪枝删除代码
                # 剪枝添加代码
                tree = PrimitiveTree(content(pset=pset.pset_list[i]))
                tree.pset = pset.pset_list[i]  # <--- 这一行很关键
                self.gene.append(tree)
                #剪枝修改代码结束
            else:
                #剪枝修改代码开始
                #self.gene.append(PrimitiveTree(content()))#剪枝删除代码
                # 剪枝添加代码
                tree = PrimitiveTree(content())
                tree.pset = pset  
                self.gene.append(tree)
                # 剪枝修改代码结束
        if tpot_model != None:
