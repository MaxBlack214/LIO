...
class EvolutionaryForestRegressor(RegressorMixin, TransformerMixin, BaseEstimator,
    ...
    def primitive_initialization(self, x):
        ...
        #     pset.addEphemeralConstant("rand101", partial(lambda: np.random.normal()))
        # elif self.constant_type is None:
        #     pass
        # elif not self.basic_primitives:
        #     pset.addEphemeralConstant("rand101", partial(lambda: random.randint(-1, 1)), NumericalFeature)
        # elif self.constant_type == 'Float':
        #     pset.addEphemeralConstant("rand101", partial(lambda: random.uniform(-1, 1)))
        # else:
        #     pset.addEphemeralConstant("rand101", partial(lambda: random.randint(-1, 1)))
    ...
    def fit(self, X, y, test_X=None):
        ...
        hof_depths = []
        for ind in self.hof:
            Yp = self.feature_generation(X, ind)
            y_pred = ind.pipe.predict(Yp)
            hof_preds.append(y_pred)
            hof_depths.extend([g.height for g in ind.gene])

        hof_preds = np.array(hof_preds)  # (n_hof, n_samples)
        avg_pred = np.mean(hof_preds, axis=0)  # HOF 平均预测结果

        best_metrics = {
            "r2": r2_score(y, avg_pred),       # 用平均预测算 R²
            "median_r2": np.median(avg_pred),  # 平均预测的中位数
            "mean_r2": np.mean(avg_pred),      # 平均预测的均值
            "num_genes": np.mean([len(ind.gene) for ind in self.hof]),  # HOF 平均基因数
            "depth_list": hof_depths,
            "depth_max": np.max(hof_depths),
            "depth_min": np.min(hof_depths),
            "depth_mean": np.mean(hof_depths),
            "hof_size": len(self.hof),
        }

        # Population 只统计深度信息，不调用 pipe.predict
        pop_depth_list = [[g.height for g in ind.gene] for ind in self.pop]
        pop_metrics = {
            "num_individuals": len(self.pop),
            "depth_list": pop_depth_list,
            "depth_max": np.max([max(d) for d in pop_depth_list]),
            "depth_min": np.min([min(d) for d in pop_depth_list]),
            "depth_mean": np.mean([np.mean(d) for d in pop_depth_list]),
        }
        ...
        run_id = time.strftime("%Y%m%d_%H%M%S")  # run_id

        # Population 数据按个体展平（只包含深度统计）
        pop_df = pd.DataFrame({
            "run_id": [run_id] * len(self.pop),
            "individual": list(range(len(self.pop))),
            "r2": ["" for _ in range(len(self.pop))],  # Population 没有 r2
            "depth_mean": [np.mean(d) for d in pop_depth_list],
            "depth_max": [np.max(d) for d in pop_depth_list],
            "depth_min": [np.min(d) for d in pop_depth_list],
            "hof_size": ["" for _ in range(len(self.pop))],
            "time_cost": ["" for _ in range(len(self.pop))],
        })

        # ⭐ 保存 HOF 平均结果
        best_df = pd.DataFrame({
            "run_id": [run_id],
            "individual": ["hof_avg"],  # 更清晰
            "r2": [best_metrics["r2"]],
            "depth_mean": [best_metrics["depth_mean"]],
            "depth_max": [best_metrics["depth_max"]],
            "depth_min": [best_metrics["depth_min"]],
            "hof_size": [best_metrics["hof_size"]],
            "time_cost": [""],
        })

        # 总耗时
        total_metrics_time = time.time() - start_metrics_time
        print(f"[Metrics] 总耗时: {total_metrics_time:.2f}s")
        ...
        summary_df = pd.DataFrame({
            ...
            "depth_min": [""],
            "hof_size": [""],
            "time_cost": [f"{total_metrics_time:.2f}s"],
        })

        final_df = pd.concat([pop_df, best_df, summary_df], ignore_index=True)

        file_exists = os.path.isfile(csv_path)
        final_df.to_csv(csv_path, mode="a", index=False, header=not file_exists)
    ...
    def predict(self, X, return_std=False, return_all=False):
        if self.normalize:
        ...
        if not return_all:
            ...
            predictions = np.array(predictions)  # (n_hof, n_samples)
            mean_prediction = np.mean(predictions, axis=0)
            std_prediction = np.std(predictions, axis=0)
            hof_size = len(self.hof)
            return final_prediction,predictions,mean_prediction,std_prediction,hof_size
